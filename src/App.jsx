...
/* ---------- tag-day: 고객사×아이템별 (해당 월, 매장 보유분 적분) ---------- */
function tagDaysDetail(stores, transactions, period) {
  ...
  const ev = {};
  const push = (s, i, dateISO, delta) => {
    if (!s || s === WAREHOUSE) return;
    const k = s + "|" + i;
  ...
  const detail = {}; // { customerId: { itemId: tagDays } }
  for (const k in ev) {
    const [storeId, itemId] = k.split("|");
    const store = storeMap[storeId];
    if (!store || !store.customerId) continue;
    ...
    if (days === 0) continue;
    if (!detail[store.customerId]) detail[store.customerId] = {};
    detail[store.customerId][itemId] = (detail[store.customerId][itemId] || 0) + days;
  }
  return detail;
}
...
function periodAccounting(items, stores, customers, contracts, transactions, period, currencies = DEFAULT_CURRENCIES, fxRates = {}) {
  ...
  for (const ct of contracts) {
    ...
    if (ct.billing === "flat") {
      ...
      let amountFx = 0, td = 0, lines = [], missingRate = false;
      for (const itemId in itemTd) {
        const it = itemMap[itemId]; if (!it) continue;
        const d = itemTd[itemId];
        const rate = Number((ct.rates || {})[itemId]) || 0; // 계약 단가표(p, 계약 통화)
        if (rate === 0 && d > 0) missingRate = true;
        const amt = d * rate;
        lines.push({ item: it, tagDays: d, dailyRate: rate, amount: amt });
        td += d; amountFx += amt;
      }
      lines.sort((a, b) => b.amount - a.amount);
      const fx = toKRW(amountFx, currency, period, currencies, fxRates);
      billItems.push({ contract: ct, customer: cust, billing: "usage", currency, amountFx, amount: fx.krw, rate: fx.rate, missingFx: !fx.ok, tagDays: td, lines, missingRate });
      leaseIncome += fx.krw;
    }
  }
  ...
  const itemTotalTd = {};
  for (const cu in detail) for (const im in detail[cu]) itemTotalTd[im] = (itemTotalTd[im] || 0) + detail[cu][im];
  const custRev = {};
  ...
  for (const cu in detail) for (const im in detail[cu]) {
    ...
    custCost[cu] = (custCost[cu] || 0) + (itemDepMap[im] || 0) * (detail[cu][im] / tot);
  }
...
function makeSample(custN = 10, storesPerCust = 20, itemN = 200) {
  ...
  for (let c = 1; c <= custN; c++) {
    ...
    const ct = { id: uid(), customerId: cust.id, contractNo: `CT-2025-${String(c).padStart(3, "0")}`,
      ...
      monthlyFee: usage ? 0 : 2000000 + (c % 4) * 500000, rates: {} };
...
export default function LeaseManager() {
  ...
  const [data, setData] = useState({ customers: [], items: [], stores: [], contracts: [], transactions: [], currencies: DEFAULT_CURRENCIES, fxRates: {}, closedPeriods: [] });
  ...
  const persist = useCallback(async (next) => {
    setData(next);
    try { await window.storage.set(STORAGE_KEY, JSON.stringify(next)); } catch (e) { flash("저장 실패 — 다시 시도해 주세요"); }
  }, []);
  ...
  if (loading) return <div className="min-h-screen grid place-items-center bg-slate-50 text-slate-400">불러오는 중…</div>;
      ...
      <main className="mx-auto max-w-6xl px-4 py-5">
        {customers.length === 0 && items.length === 0 ? (
          ...
          <>
            ...
            {tab === "txn" && <TxnView {...{ data, persist, flash, storeMap, custMap, itemMap, inventory }} />}
...
function StockView({ items, stores, customers, inventory, storeMap, custMap }) {
  ...
  const itemRows = useMemo(() => {
    ...
    for (const it of items) {
      ...
      for (const loc in locs) {
        ...
        else if (custFilter) { if (loc === WAREHOUSE || !allowed.has(loc)) continue; }
  ...
  const custRows = useMemo(() => customers
    ...
    .map((c) => {
      ...
      return { customer: c, sLines, total: sLines.reduce((a, b) => a + b.total, 0), storeCount: sLines.length };
    }).filter((r) => r.total > 0).sort((a, b) => b.total - a.total), [customers, stores, byStore, q, custFilter]);
                ...
                {itemRows.slice(0, 500).map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-2 text-slate-500 text-xs">{r.loc === WAREHOUSE ? "—" : (custMap[st?.customerId]?.name || "—")}</td>
                      <td className="px-4 py-2">
                        {r.loc === WAREHOUSE
                          ? <span className="inline-flex items-center gap-1 text-slate-500"><Warehouse size={13} /> 창고</span>
              ...
              <Card key={r.customer.id} className="overflow-hidden">
                <button onClick={() => toggleExp(r.customer.id)} className="w-full flex items-center gap-2 px-4 py-3 hover:bg-slate-50/60 text-left">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[11px] text-slate-400">{r.customer.code} · 보유 매장 {r.storeCount}</div>
                  </div>
                      ...
                      return (
                        <div key={s.store.id} className="border-b border-slate-50 last:border-0">
                          <button onClick={() => toggleExp(s.store.id)} className="w-full flex items-center gap-2 pl-9 pr-4 py-2 hover:bg-slate-50/60 text-left">
                            ...
                            <span className="flex-1 text-[13px] font-medium text-slate-600 flex items-center gap-1.5"><Store size={12} className="text-slate-400" /> {s.store.name}</span>
...
/* ---------- 입출고 ---------- */
function TxnView({ data, persist, flash, storeMap, custMap, itemMap, inventory }) {
    ...
    <div className="grid lg:grid-cols-5 gap-4">
      <Card className="p-4 lg:col-span-2 h-fit">
        ...
        <div className="space-y-3">
          ...
          {needTo && (
            <Field label={form.type === "issue" ? "불출 매장" : "도착 매장"}>
      ...
      <Card className="lg:col-span-3 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-sm font-bold">입출고 이력</h3><span className="text-xs text-slate-400">{transactions.length}건</span>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {transactions.slice(0, 300).map((t) => {
                const nm = (id) => id === WAREHOUSE ? "창고" : (storeMap[id]?.name || (t.type === "scrap" ? "폐기" : "—"));
                return (
...
function ContractMaster({ data, persist, flash, custMap, period }) {
  ...
  const [f, setF] = useState({ customerId: "", contractNo: "", startDate: todayISO(), endDate: "", billing: "flat", monthlyFee: "", currency: "KRW" });
  ...
  const add = () => {
    ...
    const ct = { id: uid(), customerId: f.customerId, contractNo: f.contractNo.trim(),
      ...
      monthlyFee: f.billing === "flat" ? (Number(f.monthlyFee) || 0) : 0, rates: {} };
    persist({ ...data, contracts: [ct, ...contracts] });
    flash("계약 등록 완료");
    setF({ customerId: f.customerId, contractNo: "", startDate: todayISO(), endDate: "", billing: "flat", monthlyFee: "", currency: f.currency });
  ...
  return (
    <div className="space-y-3">
      <Card className="p-4">
        ...
        <p className="text-[11px] text-slate-400 mt-1">월정액: 계약당 단일 정액 · 사용량: 품목코드별 일당단가(p) × 수량(q) × 일수. 종료일을 비우면 무기한 계약입니다.</p>
        <style>{`.mi{border-radius:.5rem;font-size:.875rem;padding:.5rem .625rem;background:#fff;box-shadow:inset 0 0 0 1px #cbd5e1;outline:none}.mi:focus{box-shadow:inset 0 0 0 2px #4f46e5}`}</style>
      </Card>
